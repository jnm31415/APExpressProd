  ...
  def generate_pdf(request):
      ...
      for i in range(len(pos.values('id'))):
        ...
        invoice_data += [{'pos': i+1, \
                          ...
                          'total':'{0:.{1}f}'.format(round(pos.values('anzahl')[i]['anzahl']*pos.values('einzelpreis')[i]['einzelpreis']+pos.values('anzahl')[i]['anzahl']*pos.values('einzelpreis')[i]['einzelpreis']/100*pos.values('mwst')[i]['mwst']+pos.values('pauschale')[i]['pauschale']+pos.values('pauschale')[i]['pauschale']/100*pos.values('pauschale')[i]['pauschale'],2),2) , \
...
def storno(request,pk):
  ...
  def generate_pdf(request):
      ...
      def myFirstPage(canvas, doc):
        ...
        canvas.setFont('Helvetica', 6)
        canvas.drawString(13.8*cm,21.8*cm,'*Bitte bei Schrift- und Zahlungsverkehr angeben*')
        canvas.setFont('Helvetica', 10)
